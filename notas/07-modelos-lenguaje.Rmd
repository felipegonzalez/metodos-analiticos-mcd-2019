...
de que la frase empiece сon la palabra *el*. Entonces, para calcular
...
### Cálculo de probabilidad de textos: el problema de los ceros
...
log_prob <- function(textos, n_gramas, n = 2, laplace = FALSE, delta = 0.001, vocab_env = NULL){
  ...
  df <- data_frame(id = 1:length(textos), txt = textos) %>%
         mutate(txt = normalizar(txt)) %>% 
         mutate(txt = paste(inicio, txt, sep =""))
...
- La cuarta frase tiene probabilidad 0 en todos los modelos porque la palabra *xxyz* no existe en el vocabulario 
de entrenamiento.
...
- El modelo de trigramas, también da probabilidad 0 a la última frase, lo cual también parece incorrecto
...
set.seed(413)
for(i in 1:40){
  siguiente <- calc_siguiente_bi(texto, n_gramas_u)
  texto <- c(texto, siguiente)
  if(siguiente == "_ss_"){
    texto <- c(texto, "_s_")
  }
...
el **Kneser-Ney**. Consideremos el caso de bigramas.
...
En el ajuste maximizamos la verosimilitud (por ejemplo sobre una frase):
