...
periodico_m_unk <- periodico_m %>% 
    mutate(txt_u = map_chr(txt, ~restringir_vocab(.x, vocab = vocab_env))) %>% 
    select(id, txt_u) %>% rename(txt = txt_u)
```
...
$$E = -\log \hat{P}(w_n|w_{n-1}w_{n+1}) = -y_{w_n} + \log\sum_j \exp(y_j),$$
...
$$E = -\log\sigma(y_{w_n}) + \sum_{j=1}^k \log\sigma(y_j),$$
