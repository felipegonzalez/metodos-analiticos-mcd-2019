...
Entonces el número de itemsets que obtenemos (longitud menor o igual a 7) es 
...
En este ejemplo chico, el cálculo de esta colección puede requerir hasta unos 4Gb de memoria (8Gb no son suficientes para encontrar los de tamaño 8, 9 y 10).
...
**factible**, pues podemos descartar una gran cantidad de artículos o itemsets *a priori* de manera simple, y es no es necesario contar todo.
...
```
Comparamos con los pares frecuentes caundo el soporte es 1\%:
```{r}
...
cada subconjunto de tamaño 3 está en $C_2$, y filtramos.
...
Podemos usar el modelo de independencia 
Veamos cómo se comportan confianza y lift para el modelo donde no hay asociaciones. Obsérvese
que en este modelo *todas las confianzas son iguales a la frecuencia del consecuente, y
todos los valores de lift son 1*:
...
Podemos ver cómo se relacionan lift y hyper_lift. Vemos claramente
...
Cortando en un valor relativamente bajo, vemos que nos deshacemos
...
Vemos que las reglas cortadas con hyperlift tienen mejores valores 
de soporte
...
En resumen, al descartar aquellas reglas con soporte bajo que
tienen lift alto por azar, encontramos reglas de mejor calidad en general.
