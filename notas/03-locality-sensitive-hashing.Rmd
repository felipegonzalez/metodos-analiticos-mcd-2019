...
En explicamos como hacer la búsqueda de pares
...
Empeamos con una muestra de los datos:
...
lsh_wiki_1 <- firmas_wiki %>% 
    mutate(cubeta = map(firma, crear_cubetas)) %>% 
    select(-firma) %>% unnest %>% 
    group_by(cubeta) %>% summarise(docs = list(doc_id)) %>% 
...
extraer_pares_candidatos <- function(lsh, textos = NULL){
   pares <- lsh %>% 
    mutate(pares = map(docs, ~ combn(sort(.x), 2, simplify = FALSE))) %>%
    select(cubeta, pares) %>% unnest %>% 
