...
de sus característica, ajustamos el modelo simple de gustos homogéneos (la predicción es el
...
Nótese que tenemos predicción bajo este modelo para una combinación de usuario/película, usamos el 
...
ggplot(sample_n(latentes_pelis, 500), aes(y = features_1, x = media_peli - mean(latentes_pelis$media_peli), 
...
```{r}
arrange(latentes_pelis, features_4) %>% 
  select(nombre, features_4, media_peli, num_calif_peli) %>% 
  filter(num_calif_peli > 500) %>% 
  head(100) 
arrange(latentes_pelis, features_4) %>% 
  select(nombre, features_4, media_peli, num_calif_peli) %>% 
...
for(iter in 1:15){
  ...
  salida <- descenso_estocastico(i, j, y, U, V, mu, a, b, 
              gamma = 0.003, lambda_mat = 5, lambda_sesgos = 5,
...
ggplot(sample_n(dat_entrena, 5000), aes(x= ymd(fecha), y = calif)) + 
  geom_smooth(method = 'loess', span=0.3, lambda = 1)
```
...
ggplot(filter(medias_pelis, num_calif_peli > 1000), 
  ...
  geom_point(alpha=0.4) + geom_smooth(method = 'loess', lambda = 1, span=0.4)
```
...
un tamaño razonable de de datos de validación.
